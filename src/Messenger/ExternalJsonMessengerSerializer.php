...
class ExternalJsonMessengerSerializer implements SerializerInterface
{
    public function decode(array $encodedEnvelope): Envelope
    {
        ...
        if (null === $data) {
            throw new MessageDecodingFailedException('Invalid JSON data');
        }

        if (!isset($data['emoji'])) {
            throw new MessageDecodingFailedException('Missing "emoji" field in the message data');
        }
    ...
    private function createLogEmojiEnvelope(array $data): Envelope
    {
        ...
        $message = new LogEmoji($data['emoji'] ?? 0);

        $envelope = new Envelope($message);

        // need only if you need this to be sent through the non-default bus
